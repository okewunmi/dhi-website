...
export default function HomeHero({ data }: { data: HeroData }) {
  ...
  return (
    <section className="relative bg-white overflow-hidden min-h-[92vh] flex items-center">
      ...
      {/* mt-[64px] on mobile matches navbar height, lg:mt-0 since section is min-h-screen */}
      <div className="container-dhi relative z-10 py-12 mt-[80px] md:mt-[90px] lg:mt-0 lg:py-0">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center">

          {/* Text */}
          <div className="animate-fade-up">
            <span className="section-label mb-4  md:mb-6 mt-14 ">Da Hausa Initiative</span>

            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-5xl font-light text-black mb-5 md:mb-8 leading-[1.1]">
              {lines.map((line, i) => (
                <span key={i} className={i > 0 ? "block text-[#BF4E14]" : "block"}>
                  {line}
                </span>
            ...
            <p className="text-sm sm:text-base text-[#4A4A4A] mb-7 md:mb-10 max-w-lg leading-relaxed">
              {data.subheadline ||
                "Da Hausa Initiative works to improve financial and data literacy across Hausa-speaking communities in Northern Nigeria."}
            </p>

            <div className="flex flex-wrap gap-3 md:gap-3">
              <Link href={data.cta_primary_link || "/apply"} className="btn-primary text-xs sm:text-sm md:text-base px-3 py-2 sm:px-3 sm:py-3">
                {data.cta_primary_text || "Apply Now"}
                <ArrowRight size={15} />
              </Link>
              <Link href={data.cta_secondary_link || "/programmes"} className="btn-secondary text-xs sm:text-sm md:text-base px-3 py-2 sm:px-3 sm:py-3">
                {data.cta_secondary_text || "Our Programmes"}
              </Link>
            </div>

            {/* Stats row */}
            <div className="flex gap-6 sm:gap-10 mt-8 md:mt-14 pt-6 md:pt-10 border-t border-[#E8E8E8]">
              {[
                { label: "Programmes", value: "2 Pillars" },
                { label: "Approaches", value: "3 Methods" },
                { label: "Focus", value: "N. Nigeria" },
              ].map((stat) => (
                <div key={stat.label}>
                  <p className="text-base sm:text-xl font-semibold text-[#BF4E14]">{stat.value}</p>
                  <p className="text-[10px] sm:text-xs uppercase tracking-wide text-gray-400 mt-0.5">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Visual panel */}
          <div className="animate-fade-up-delay-1 relative">
            <div className="relative aspect-[4/3] sm:aspect-[4/4] lg:aspect-[4/5] bg-[#F5F5F5] overflow-hidden">
              {data.image_url ? (
                <Image
                  src={data.image_url}
                  alt="Da Hausa Initiative"
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-6 md:p-10">
                  <div className="w-16 md:w-24 h-1 bg-[#BF4E14] mb-6 md:mb-8" />
                  <div className="text-center">
                    <p className="font-display text-2xl md:text-4xl font-light text-black mb-2 md:mb-3">Financial</p>
                    <p className="text-xs sm:text-sm uppercase tracking-widest text-gray-400 mb-4 md:mb-6">&</p>
                    <p className="font-display text-2xl md:text-4xl font-light text-[#BF4E14]">Data Literacy</p>
                  </div>
                  <div className="w-16 md:w-24 h-1 bg-[#BF4E14] mt-6 md:mt-8" />
                  <p className="text-[10px] md:text-xs text-gray-400 uppercase tracking-widest mt-6 md:mt-8 text-center">
                    Research · Training · Advocacy
                  </p>
                </div>
              )}
            ...
            {/* Floating card — tucked in tighter on mobile */}
            <div className="absolute -bottom-4 -left-3 sm:-bottom-6 sm:-left-6 bg-[#BF4E14] text-white p-5 sm:p-8 shadow-xl max-w-[160px] sm:max-w-[220px]">
              <p className="text-lg sm:text-2xl font-display font-light mb-1">Northern</p>
              <p className="text-[10px] sm:text-xs uppercase tracking-widest opacity-70">Nigeria Focus</p>
            </div>
          </div>
