...
import { Save, Upload, ChevronDown, ChevronUp, Plus, Trash2, ImageIcon } from "lucide-react";
...
// Renders a field editor based on value type
function FieldEditor({
  ...
  // Array of objects (programmes, pillars, approaches, etc.)
        ...
        {(value as Record<string, unknown>[]).map((item, i) => (
          <div key={i} className="border border-[#E8E8E8] p-4 bg-[#FAFAFA] relative">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold uppercase text-[#BF4E14] tracking-wide">
                Item {i + 1}
              </span>
              <button
                onClick={() => {
          ...
          onClick={() => {
            ...
            const newItem = Object.fromEntries(
              Object.keys(template).map((k) => [
                k,
                typeof template[k] === "boolean" ? false : "",
              ])
            );
        ...
        <div className="border-t border-[#E8E8E8] p-6">
          <div className="space-y-6">
            {Object.entries(data).map(([key, val]) => (
              <div key={key}>
                <label className="label-dhi mb-2 block">
                  {key.replace(/_/g, " ")}
                </label>
                <FieldEditor
                  fieldKey={key}
