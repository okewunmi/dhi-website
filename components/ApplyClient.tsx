...
export default function ApplyClient({
  ...
  applyPageIntro: string;
}) {
  const openProgrammes = programmes.filter((p) => p.status !== "closed");
  const [activeId, setActiveId] = useState<string | undefined>(openProgrammes[0]?.id);
  const active = openProgrammes.find((p) => p.id === activeId);
    ...
    <>
      <section className="bg-[#F5F5F5] py-20 lg:py-28">
        <div className="container-dhi">
          ...
          <p className="text-[#4A4A4A] text-lg max-w-2xl">{applyPageIntro}</p>
        </div>
      </section>
      ...
      <section className="section-padding bg-white">
        <div className="container-dhi">
          {openProgrammes.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-[#4A4A4A] text-lg mb-6">
                Applications are currently closed. Check back soon or subscribe to our
                ...
                {active && (
                  <div>
                    <div className="mb-8">
                      ...
                      <p className="text-[#4A4A4A] text-sm">{active.description}</p>
                    </div>
                    ...
                    {active.status === "coming_soon" ? (
                      ...
                      <div className="text-center py-12 border border-[#E8E8E8]">
                        <p className="text-[#4A4A4A] mb-6 text-sm">
                          Applications for this programme are handled via Google Form.
                        </p>
                        <a
                          href={active.google_form_link}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="btn-primary inline-flex items-center gap-2"
                        >
                      ...
                      <p className="text-sm text-gray-400">No application link set yet.</p>
                    )}
                  </div>
                )}
              </div>
